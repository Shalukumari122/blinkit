...
from blinkit.items import Blinkit_comp
...
class BlinkitMultiPinSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # query1 = "SELECT id, status FROM blinkit_links_comp"
        ...
        #     id, status = row
        #     if status == "Done":
        #         # Update status to 'pending' for the specific row
        #         update_status = """
        #             UPDATE blinkit_links_comp
        #             SET status = 'pending'
        #             WHERE id = %s
        #         """
        #         self.cur.execute(update_status, (id,))
        #         self.conn.commit()
        #         print(f"Row with ID {id} updated successfully.")
        #     else:
        #         print(f"Row with ID {id} status is already pending.")
        ...
        folder_loc = f'C:/paga_save/live_project/blinkit_weekly/comp/{today_date}/'
